...
def get_vod_list():
    ...
    if keyword:
        movs = movs.filter(or_(MovDetail.vod_name.like(f"%{keyword}%"),
    ...
    return jsonify(dict(code=200, message="success", data=vod_list))
...
def get_vod_detail():
    ...
    return jsonify(dict(code=400, msg='failed no this mov'))
